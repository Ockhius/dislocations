...
def do_validate(epoch, model, val_loader, loss_func, tf_logger):
    ...
    model.eval()
    total_test_loss = 0
    for batch_idx, (l, lgt, l_name) in enumerate(val_loader):

        with torch.no_grad():
            l, lgt = l.cuda(), lgt.cuda()

            seg_map_l, seg_l = model(l)

            loss = loss_func(seg_l, lgt)
            total_test_loss += loss.item()

            seg_l, lgt, l = seg_l.detach(), lgt.detach(), l.detach()
            seg_l = torch.sigmoid(seg_l) >0.5
...
def do_train(
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            for batch_idx, (l, lgt, l_name) in enumerate(train_loader):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--config_file", default="/cvlabsrc1/cvlab/datasets_anastasiia/dislocations/dislocations/delineation/configs/dislocation_segmentation_only.yml", help="path to config file", type=str
    )
