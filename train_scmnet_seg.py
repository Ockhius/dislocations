...
def do_validate(seg_model, model, val_loader, loss_func):
    ...
    for batch_idx, (l, r, lgt, _, dlgt, l_name) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            dl_scores = model(l, r, l_segmap, r_segmap)
...
def train(cfg):
    ...
    # create model
    seg_model, model = build_model_list(cfg, [True, False])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--config_file", default="delineation/configs/dislocation_matching_seg_home.yml", help="path to config file",
        type=str
    )
