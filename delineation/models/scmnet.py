...
class SCMNET(nn.Module):
    def __init__(self, in_channels, out_channels, num_block, maxdisp=32, disp_space='two-sided'):
        ...
        self.res_block = self._make_layer(res_2d_block, self.in_planes, 32, num_block[0], stride=1)

        self.conv1 = nn.Conv2d(32, 32, 5, 2, 2)
    ...
    def forward(self, left_im, right_im):
        ...
        left_fmap = F.relu(self.bn0(self.conv0(left_im)))
        ...
        # cost volume
        cv = cost_volume(right_fmap, left_fmap, maxdisp=self.maxdisp, disp_space=self.disp_space)

        conv3d_out = F.relu(self.bn3d_1(self.conv3d_1(cv)))
        conv3d_out = F.relu(self.bn3d_2(self.conv3d_2(conv3d_out)))

        conv3d_block_1 = self.block_3d_1(cv)
        conv3d_block_2 = self.block_3d_2(conv3d_block_1)

        # deconv
        deconv3d = F.relu(self.debn1(self.deconv1(conv3d_block_2)) + conv3d_block_1)
        deconv3d = F.relu(self.debn2(self.deconv2(deconv3d)) + conv3d_out)
