...
class MatchingDislocationsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        left_seg_img = Image.open(os.path.join(self.root_dir, self.split, self.subset_folders['S_L'], self.l_images_path[idx])).convert('L')
        right_seg_img = Image.open(os.path.join(self.root_dir, self.split, self.subset_folders['S_R'], self.l_images_path[idx].replace('LEFT','RIGHT'))).convert('L')
        ...
        left_seg_img = torch.from_numpy(left_seg_img).float().unsqueeze(0)
        right_seg_img = torch.from_numpy(right_seg_img).float().unsqueeze(0)
